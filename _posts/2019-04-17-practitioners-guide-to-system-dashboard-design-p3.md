...
Be careful to avoid [moiré patterns](https://en.wikipedia.org/wiki/Moir%C3%A9_pattern). This can happen when your bars are too small and evenly spaced. Include a little space between bars to aid the visual separation.
...
# Heat Maps
Reach for heap maps when you have a line chart that has too many lines to make sense of. Heat maps vary intensity of color such that a lot of data can be packed in to a small space. Occlusion hurts line charts when there are too many lines. Heat maps excel here, allowing you to find bands of common intensity to spot pattern. This can often point to patterns or outliers.
...
Use these types of visualizations when your metric has a basis for comparison, such as SLOs!
...
* Donut charts: Eh, this are mostly cute ways of showing a single value.
...
* Sparklines: Great to supplement single values, but often lack
...
* Dials and gauges:  If your data does not actually have lower and upper-bounds, presenting it in a dial is misleading

# Summary

Before you flame me for missing some weird visualization or an option that your tool has, let me remind you that if your user understands it then it's good. As your use case broadens then so will your user base and we fall back on the guidance from [Part 2](TKTK) to choose our visualizations.
